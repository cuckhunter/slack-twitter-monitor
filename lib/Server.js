...
<div class="high-scoring-tweets-task">
  ...
  Mentions: ({ screenName, name, total, lastTweet, time }) => {

    return `
<div class="mentions-task">
  <p><strong>Mentions of ${name} (@${screenName}}</strong></p>
  <p>Total tweets: ${total}</p>
  <p>Last tweet: ${renderTweetLink(lastTweet)} ${renderTweetTime(time)}</p>
</div>`;
  },
...
function getServer([$env, $logger, $slackService, $taskService, $twitterService]) {
  ...
  return http.createServer(async (req, res) => {
    ...
    const matchNumberle = /^\/numberle/i.test(req.url);

    if (match) {

      const result = await $twitterService.getTweet({ id: match[1], protected: true, tweet_mode: 'extended' });
      res.end(renderTest(result));

    } else if (matchNumberle) {

      const filename = path.join($env.HOME, 'numberle.html');
