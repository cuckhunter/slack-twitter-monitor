  ...
  async runPredictItTask([$slackService], id) {
    ...
    const execute = setPoll(async () => {
      ...
      const body = await new Promise((resolve, reject) => {
        ...
        request('https://www.predictit.org/api/marketdata/all/', (err, response, body) => {
          if (err) {
      ...
      const market = data.markets.find((market) => market.id === id);
      let result = `*<${market.url}|${market.shortName}>*\n`;
      for (const contract of market.contracts) {
        result += `${contract.name}: ${Math.round(contract.lastTradePrice * 100)}%\n`;
      }

      await $slackService.message(result);
