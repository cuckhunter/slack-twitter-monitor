...
const KEYWORDS_REG_EXP = new RegExp([
  ...
  'power[\\- ]?law',
  'power[\\- ]?level',
  // add this back in // '(?:(?:all the )?(?:hall|ear)marks of (?:a )?)?russian disinfo(?:rmation)?(?: (?:campaign|op(?:eration)?))?', // has to precede 'information op'
...
class TaskService {
  ...
  async runLunaTask([$slackService]) {
    ...
    const execute = setPoll(async () => {
      ...
      await $slackService.message({
        username: 'terra.dev',
        text: `Circulating supply: ${Number((~~body).toFixed(2)).toLocaleString()}`,
      });
      ...
      if (newTweets.length) {
        ...
        for (let i = 0; i < newTweets.length; i++) {
          ...
          if (
            ...
            (tweet.user.screen_name != 'would_it_dong' || /IT'S A UNICORN|Home Run[^]*\D[1-3]\/30|(?<!Home Run[^]*)2[7-9]\/30/.test(tweet.full_text)) &&
