      ...
      if (newTweets.length) {
        ...
        for (let i = 0; i < newTweets.length; i++) {
          ...
          if (
            ...
            // Include context tweet if it is not by user and does not mention user
            if (includeContext && tweet.in_reply_to_screen_name && tweet.in_reply_to_screen_name != screenName) {
              const parentTweet = await $twitterService.getTweet({ id: tweet.in_reply_to_status_id_str });
              if (!parentTweet.entities.user_mentions.some((mention) => mention.screen_name == screenName)) {
                tweets.push(tweet.in_reply_to_status_id_str);
              }
            }
          ...
          if (
            ...
            if (tweet.referenced_tweets) {
              ...
              if (retweet) {
                ...
                const isReply = !!parentTweet.in_reply_user_id;
                const isQuoteTweetOfUser = parentTweet.referenced_tweets
                  .filter((item) => item.type == 'quoted')
                  .some((referencedTweet) => includes.tweets.find((fullTweet) => referencedTweet.id == fullTweet.id).author_id == userId);
