      ...
      if (newTweets.length) {
        ...
        for (let i = 0; i < newTweets.length; i++) {
          ...
          if ((tweet.user.statuses_count - i) % modulus === 0) {
            if (
              tweet.user.screen_name != 'RealSkipBayless' || !tweet.text.match(/undisputed/i)
              //tweet.entities.user_mentions.every(mention => (mention.screen_name.toLowerCase() != 'undisputed'))
            ) {
              tweets.push(tweet);
            }
          }
