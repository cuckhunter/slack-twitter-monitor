...
class TaskService {
   ...
   * @param  {String} screenName
   */
  async runHighScoringTweetsTask([$env, $slackService, $twitterService], screenName, interval) {

    const MAX_DELAY = 5 * 60 * 1000;
    const MIN_SCORE = 0.00001;
    ...
    const getScore = function ({
      ...
      return followerCount > 0 ? (favoriteCount + 3 * retweetCount) / followersCount : 0;
    }
    ...
    const execute = setPoll(async () => {
      ...
      if (newTweets.length) {
        ...
        for (let i = newTweets.length - 1; i >= 0; i--) {
          ...
          let tweet = newTweets[i];
          let age = now - new Date(tweet.created_at);
