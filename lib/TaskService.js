...
class TaskService {
  ...
  async runMentionsTask(
    ...
    const { name } = await $twitterService.getUser({ screen_name: screenName });
    ...
    // let sinceId = initialTweets.data.length ? initialTweets.data[$env.MODE == 'prod' ? 0 : 1].id : undefined;
    let sinceId = initialTweets.data.length ? initialTweets.data[1].id : undefined;
    let total = 0;
    let lastTweet;
    let time;
    ...
    const execute = setPoll(async () => {
      ...
      const { data: newTweets, includes } = await $twitterService.getMentions(screenName, { sinceId });
      ...
      if (newTweets.length) {
        ...
        for (let i = 0; i < newTweets.length; i++) {
          ...
          if (
            ...
            tweet.user = { screen_name: includes.users.find((item) => item.id == tweet.author_id) };
            tweets.push(tweet);
          }
        }
