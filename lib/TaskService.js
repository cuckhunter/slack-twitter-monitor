...
class TaskService {
  ...
  async runTrumpDeskTask([$slackService]) {
    ...
    const interval = 30 * 1000; //10 * 60 * 1000;
    ...
    let lastId = cheerio.load(body)('.ftdli').eq(2).data('id');
    let time;
    ...
    const execute = setPoll(async () => {
      ...
      $posts.each((index, item) => {
        ...
        if ($post.data('id') > startId) {
          results.push({
            text: $post('.ftd-post-text').text(),
            imgs: $post('.ftdli-main-content img').map((index, item) => cheerio(item).attr('src')).get(),
            date: $post('.date p').text(),
          });
        }
      });
      
      startId = $posts.data('id');
      ...
      const result = results.map((item) => {
        ...
        elements.push(...item.imgs);
        elements.push(`_${item.date}_`);
