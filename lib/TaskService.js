...
const KEYWORDS_REG_EXP = new RegExp('(?<!@\\w*|://\\S*)(?:' + [
  ...
  'fag',
  'fultz',
  'global(?:ist|ism| elite)',
  '\\bhimmler|mengele|g[öo]e?ring|ilse koch|berchtold|bormann|heydrich',
  'anschluss|dolchsto(?:ß|ss)|ehrenarier|einsatzgruppen|gleichschaltung|f[üu]hrerprinzip|judenfrei|lebensborn|lebensraum|endsieg|gro(?:ß|ss)e l[üu]ge',
  'goy',
  '\\bhag',
  'haram\\b',
  'hillary',
...
class TaskService {
  ...
  async runHighScoringTweetsTask([$env, $logger, $slackService, $twitterService], screenName, level) {
    ...
    if (!initialTweets[0]) {
      ...
      await $slackService.message({
        username: 'debug',
        text: `screenName: ${screenName}, initialTweets: ${JSON.stringify}`,
      });
    }
        ...
        for (let i = newTweets.length - 1; i >= 0; i--) {
          ...
          if (age < MAX_AGE) {
            ...
            for (const item of [retweet || tweet, quotedTweet]) {
              ...
              const text = filterTweetText(item);
              let match;

              if (CAPS_REG_EXP.test(text)) {
                matchesMap['<ALL CAPS>'] = (~~matchesMap['<ALL CAPS>']) + 1;
              }

              while (match = KEYWORDS_REG_EXP.exec(text)) {
                matchesMap[match[0]] = (~~matchesMap[match[0]]) + 1;
              }

            }

            if (score >= MIN_SCORE || Object.keys(matchesMap).length) {
              if (!published[tweet.id_str]) {

                const matches = Object.entries(matchesMap).map(([key, val]) => {
                  return (val > 1) ? `${key} (${val})` : key;
                });
