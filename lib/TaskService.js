...
function filterTweetText({ full_text: text }) {

  return TWEET_FILTERS.reduce((memo, filter) => filter(tweet, memo), text);
      ...
      if (newTweets.length) {
        ...
        for (let i = newTweets.length - 1; i >= 0; i--) {
          ...
          if (age < MAX_DELAY) {
            ...
            const score = getScore(retweet || tweet, level);
            const matches = [];

            for (const item of [tweet, quotedTweet]) {
