...
const KEYWORDS_REG_EXP = new RegExp([
  '1488|anschluss|dolchsto(?:ß|ss)|ehrenarier|einsatzgruppen|endsieg|f[üu]hrerprinzip|gleichschaltung|gro(?:ß|ss)e l[üu]ge|judenfrei|lebensborn|lebensraum|l[üu]genpresse|kristallnacht|night of the long knives',
  'accelerationis[mt]',
  '(?:amy )?wax',
  ...
  '(enter(?:s|ed)?|leaves?|left) the chat',
  'etoro',
  'exploding offer',
  'eugenics?|dysgenics?',
  'evola',
  ...
  '[ny]imby',
  '(absolutely |literally )?no(?:body| ?one):',
  'nofap',
  'normies?',
  '\\bnow do\\b',
      ...
      if (newTweets.length) {
        ...
        for (let i = newTweets.length - 1; i >= 0; i--) {
          ...
          if (!published[tweet.id_str]) {
            ...
            if (tweet.user.screen_name == 'ggreenwald' && tweet.lang == 'pt') {

              await $slackService.message({
                channel: '#slack-twitter-monitor',
                username: 'debug',
                text: '<@U24PPP3BK> suppressed ggreenwald pt ' + tweet.id_str,
              });

            } else if (score >= MIN_SCORE || matches.length) {
