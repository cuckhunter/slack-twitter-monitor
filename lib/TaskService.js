...
class TaskService {
   ...
   * @param  {Number} (modulus)
   */
  async runAllMentionsTask(
    [$env, $logger, $slackService, $twitterService],
    screenName,
    modulus = 1,
  ) {

    const initialTweets = await $twitterService.getMentionsTimeline({
      screen_name: screenName,
      include_rts: true,
      count: 2,
    });
    ...
    const execute = setPoll(async () => {
      ...
      const newTweets = await $twitterService.getMentionsTimeline({
        screen_name: screenName,
        include_rts: true,
        since_id: sinceId,
      });
      ...
      if (newTweets.length) {
        ...
        if (tweets.length) {
          await $slackService.publish(tweets, `(Reply to: ${screenName})`);
          lastTweet = tweets.slice().pop();
          time = Date.now();
          total += tweets.length;
        }
