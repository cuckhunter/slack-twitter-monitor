      ...
      if (newTweets) {
        ...
        for (let i = 0; i < newTweets.length; i++) {
          ...
          if (
            ...
            if (tweet.in_reply_to_user_id && includes.users.find((item) => item.id == tweet.in_reply_to_user_id).username == screenName) {
              await $slackService.message({
                channel: 'logging',
                username: 'debug',
                text: `eliminated tweet ${tweet.id} in reply to ${screenName}`,
              });
              continue;
            }
