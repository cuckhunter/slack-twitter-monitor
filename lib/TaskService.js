...
const KEYWORDS_REG_EXP = new RegExp([
  ...
  'power[\\- ]?law',
  'power[\\- ]?level',
  '(?:(?:all the )?hallmarks of (?:a )?)?russian disinfo(?:rmation)?(?: (?:campaign|op))?', // has to precede 'information op'
...
function getScore({
  ...
  const score = (protectedUser ?
    (8 * favoriteCount) :
    ((favoriteCount + 3 * retweetCount) * adjustment)) / 3;
