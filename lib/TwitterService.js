...
function getTwitterService([$env, $logger]) {
  ...
  api.doPost = function(url, params) {
    const twitter = apps[index++ % apps.length];
    ...
    return new Promise((resolve, reject) => {
      ...
      twitter.doPost(
        ...
        function success(data) {
          ...
          if (data.length) {
            $logger.info('Twitter client found new tweets:', JSON.stringify(data, null, 2));
          }
          resolve(data);
        }
      );
  ...
  for (const method of ['getUser', 'getUserTimeline', 'getSearch', 'getHomeTimeline', 'getTweet']) {
    ...
    api[method] = function(params) {
      ...
      const twitter = (params.protectedUser) ? protectedApp : apps[index++ % apps.length];
