...
async function app([$env, $logger, $server, $slackService, $taskService]) {
  ...
  try {
    ...
    const [ALL, ALL_WITH_CONTEXT, ALL_EXCLUDE_REPLIES, HIGH, DEFAULT, LOW, NONE] = [...Array(7).keys()];
    ...
    const settings = {
      ...
      'ScottMGreer': ALL_EXCLUDE_REPLIES,
      'MaxNordau': LOW,
      ...
      // 'Ximerican': DEFAULT, // suspended
      // 'GovernorXi': DEFAULT, // suspended
    ...
    for (const [key, value] of Object.entries(settings)) {
      ...
      switch (value) {
        ...
        case ALL_EXCLUDE_REPLIES:
          ...
          break;
        default:
          tasks.push($taskService.runHighScoringTweetsTask(key, value.map(levelMap)));
          break;
      }
