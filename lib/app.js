...
async function app([$env, $logger, $server, $slackService, $taskService]) {
  ...
  try {
    ...
    await $slackService.message({
      channel: 'logging',
      username: 'debug',
      text: 'app started, host' + $env.HOST,
    });
    ...
    //   const response = await axios.get($env.HOST);
    //   $slackService.message({
    //     channel: 'logging',
    //     username: 'debug',
    //     text: `pinging: ${$env.HOST}, ${JSON.stringify(response.data, null, 2).slice(0, 350)}`,
    //   });
