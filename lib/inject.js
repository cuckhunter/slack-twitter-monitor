...
function inject(fn) {
  ...
  if (body.type == 'ClassExpression') {
    ...
    for (let method of body.body.body) {
...
function parseDependencies(params) {
  ...
  for (let element of params[0].elements) {
...
function createdInjectedFn(fn, deps) {
  ...
  for (let dep of deps) {
    ...
    const handler = {
      ...
      has: (target, property) => {
        return property in REGISTRY[dep];
      },
      ...
      get: (target, property, receiver) => {
        ...
        return REGISTRY[dep][property];
      },

      set: (target, property, value, receiver) => {
        REGISTRY[dep][property] = value;
        return true;
      }

    };

    args.push(new Proxy(TARGET, handler));
